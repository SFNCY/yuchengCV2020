 ...
 * scale-space pyramid
 */
static void calcLayerDetAndTrace(const cv::Mat &sum, int size, int sampleStep,
                                 cv::Mat &det, cv::Mat &trace)
{
 ...
 */
static int
interpolateKeypoint(float N9[3][9], int dx, int dy, int ds, cv::KeyPoint &kpt)
{
...
struct SURFBuildInvoker : cv::ParallelLoopBody
{
    SURFBuildInvoker(const cv::Mat &_sum, const std::vector<int> &_sizes,
                     const std::vector<int> &_sampleSteps,
                     std::vector<cv::Mat> &_dets, std::vector<cv::Mat> &_traces)
    {
        sum = &_sum;
        sizes = &_sizes;
    ...
    std::vector<cv::Mat> *dets;
    std::vector<cv::Mat> *traces;
};

// Multi-threaded search of the scale-space pyramid for keypoints
struct SURFFindInvoker : cv::ParallelLoopBody
{
    SURFFindInvoker(const cv::Mat &_sum, const cv::Mat &_mask_sum,
                    const std::vector<cv::Mat> &_dets, const std::vector<cv::Mat> &_traces,
                    const std::vector<int> &_sizes, const std::vector<int> &_sampleSteps,
                    const std::vector<int> &_middleIndices, std::vector<cv::KeyPoint> &_keypoints,
    ...
    static void findMaximaInLayer(const cv::Mat &sum, const cv::Mat &mask_sum,
                                  const std::vector<cv::Mat> &dets, const std::vector<cv::Mat> &traces,
                                  const std::vector<int> &sizes, std::vector<cv::KeyPoint> &keypoints,
    ...
    const cv::Mat *sum;
    const cv::Mat *mask_sum;
    const std::vector<cv::Mat> *dets;
    const std::vector<cv::Mat> *traces;
    const std::vector<int> *sizes;
    const std::vector<int> *sampleSteps;
    ...
    static cv::Mutex findMaximaInLayer_m;
};

cv::Mutex SURFFindInvoker::findMaximaInLayer_m;

/*
 * Find the maxima in the determinant of the Hessian in a layer of the
 * scale-space pyramid
 */
void SURFFindInvoker::findMaximaInLayer(const cv::Mat &sum, const cv::Mat &mask_sum,
                                        const std::vector<cv::Mat> &dets, const std::vector<cv::Mat> &traces,
                                        const std::vector<int> &sizes, std::vector<cv::KeyPoint> &keypoints,
                                        int octave, int layer, float hessianThreshold, int sampleStep)
{
        ...
        for (int j = margin; j < layer_cols - margin; j++)
        {
            ...
            if (val0 > hessianThreshold)
            {
                    ...
                    val0 > N9[2][6] && val0 > N9[2][7] && val0 > N9[2][8])
                {
                    ...
                    cv::KeyPoint kpt(center_j, center_i, (float)sizes[layer],
...
static void fastHessianDetector(const cv::Mat &sum, const cv::Mat &mask_sum, std::vector<cv::KeyPoint> &keypoints,
                                int nOctaves, int nOctaveLayers, float hessianThreshold)
{
    ...
    parallel_for_(cv::Range(0, nTotalLayers),
                  SURFBuildInvoker(sum, sizes, sampleSteps, dets, traces));

    // Find maxima in the determinant of the hessian
    parallel_for_(cv::Range(0, nMiddleLayers),
...
struct SURFInvoker : cv::ParallelLoopBody
{
    enum
    {
        ORI_RADIUS = 6,
    ...
    void operator()(const cv::Range &range) const CV_OVERRIDE
    {
        ...
        for (k = k1; k < k2; k++)
        {
            ...
            if (upright == 0)
            {
                ...
                phase(cv::Mat(1, nangle, CV_32F, X), cv::Mat(1, nangle, CV_32F, Y), cv::Mat(1, nangle, CV_32F, angle), true);
            ...
            cv::Mat win(win_size, win_size, CV_8U, winbuf);
                                       ...
                                       cv::OutputArray _descriptors)
{
    ...
    int imgtype = _image.type(),
        imgcn = CV_MAT_CN(imgtype);
    bool doDescriptors = _descriptors.needed();

    CV_Assert(!_image.empty() && CV_MAT_DEPTH(imgtype) == CV_8U && (imgcn == 1 || imgcn == 3 || imgcn == 4));
    CV_Assert(_descriptors.needed());

#ifdef HAVE_OPENCL
    if (cv::ocl::useOpenCL() && _image.isUMat())
    {
        cv::xfeatures2d::SURF_OCL ocl_surf;
        cv::UMat gpu_kpt;
        bool ok = ocl_surf.init(hessianThreshold,
                                nOctaves,
                                nOctaveLayers,
                                extended,
                                upright);
        ...
        if (ok)
        {
            ...
            {
                ok = ocl_surf.detect(_image, _mask, gpu_kpt);
            }
            else
            {
                if (useProvidedKeypoints)
                    ocl_surf.uploadKeypoints(keypoints, gpu_kpt);
                ok = ocl_surf.detectAndCompute(_image, _mask, gpu_kpt, _descriptors, useProvidedKeypoints);
            }
        }
        if (ok)
        {
    ...
    cv::Mat img = _image.getMat(), mask = _mask.getMat(), mask1, sum, msum;
    ...
    if (N > 0)
    {
        ...
        if (doDescriptors)
        {
            _1d = _descriptors.kind() == cv::_InputArray::STD_VECTOR && _descriptors.type() == CV_32F;
            if (_1d)
            {
        ...
        parallel_for_(cv::Range(0, N), SURFInvoker(img, sum, keypoints, descriptors, extended, upright));
        ...
        if (N > j)
        {
            ...
            if (doDescriptors)
            {
                cv::Mat d = descriptors.rowRange(0, N);
                if (_1d)
